	...
	public void SingleQueryTest() {
		try {
			for (int i = 0; i < Ncase; i++) {
				Main.main(new String[] { "src/test/resources/XQuery%d.txt".formatted(i), "target/test-classes/XQuery%d_result.xml".formatted(i) });
			}
		} catch (Exception e) {
	...
	public void ContentCompareTest() {
		try {
			for (int i = 0; i < Ncase; i++) {
				...
				InputStream RefInStream = new FileInputStream("src/test/resources/XQuery%d_result_standard.xml".formatted(i));
